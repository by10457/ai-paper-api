...
CREATE TABLE IF NOT EXISTS `paper_orders` (
  ...
  `outline_record_id` INT NULL COMMENT '大纲记录',
  `user_id` INT NOT NULL COMMENT '用户',
  PRIMARY KEY (`id`),
  ...
  CONSTRAINT `fk_paper_orders_outline_record_id` FOREIGN KEY (`outline_record_id`) REFERENCES `paper_outline_records` (`id`) ON DELETE SET NULL
...
CREATE TABLE IF NOT EXISTS `paper_generation_tasks` (
  ...
  `process_events` JSON NULL COMMENT '论文生成过程事件',
  `process_metadata` JSON NULL COMMENT '论文生成过程关键数据',
  `result_summary` JSON NULL COMMENT '论文生成结果摘要',
  `storage_provider` VARCHAR(32) NULL COMMENT '主存储类型',
  `file_key` VARCHAR(512) NULL COMMENT '主存储文件 key',
  `local_file_key` VARCHAR(512) NULL COMMENT '本地兜底文件 key',
  `last_error` VARCHAR(500) NULL COMMENT '最近一次错误',
  ...
  `next_retry_at` DATETIME(6) NULL COMMENT '下次自动重试时间',
  `user_id` INT NOT NULL COMMENT '用户',
  `order_id` INT NOT NULL COMMENT '关联论文订单',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uid_paper_generation_tasks_task_id` (`task_id`),
  UNIQUE KEY `uid_paper_generation_tasks_user_id_idempotency_key` (`user_id`, `idempotency_key`),
  KEY `idx_paper_generation_tasks_user_id` (`user_id`),
  KEY `idx_paper_generation_tasks_order_id` (`order_id`),
  KEY `idx_paper_generation_tasks_status` (`status`),
  KEY `idx_paper_generation_tasks_status_next_retry_at` (`status`, `next_retry_at`),
  CONSTRAINT `fk_paper_generation_tasks_user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_paper_generation_tasks_order_id` FOREIGN KEY (`order_id`) REFERENCES `paper_orders` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='论文生成任务';
...
CREATE TABLE IF NOT EXISTS `model_call_logs` (
  ...
  `call_type` VARCHAR(32) NOT NULL DEFAULT 'text' COMMENT '调用类型',
  `task_id` VARCHAR(64) NULL COMMENT '生成任务 ID',
  `stage` VARCHAR(64) NULL COMMENT '生成阶段',
  `provider` VARCHAR(64) NOT NULL COMMENT '调用协议/服务商标识',
  `model_name` VARCHAR(128) NOT NULL COMMENT '模型名称',
  ...
  `response_chars` INT NOT NULL DEFAULT 0 COMMENT '输出字符数',
  ...
  CONSTRAINT `fk_model_call_logs_generation_task_id` FOREIGN KEY (`generation_task_id`) REFERENCES `paper_generation_tasks` (`id`) ON DELETE SET NULL,
